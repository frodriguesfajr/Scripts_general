    ...
    if size(activeChnList, 2) > 3
        ...
        [navSolutions_latitude(currMeasNr), ...
         ...
         navSolutions_height(currMeasNr)] = cart2geo(...
                                            ...
                                            5);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % function [phi_geo, lambda_geo, h_geo] = cart2geo(X_geo, Y_geo, Z_geo, i_geo)
        % ==========================================================================
        X_geo = navSolutions_X(currMeasNr);
        Y_geo = navSolutions_Y(currMeasNr);
        Z_geo = navSolutions_Z(currMeasNr);
        % disp(Z_geo)
        i_geo = 5;
        a_geo = [6378388 6378160 6378135 6378137 6378137];
        f_geo = [1/297 1/298.247 1/298.26 1/298.257222101 1/298.257223563];
        lambda_geo = atan2(Y_geo,X_geo);
        ex2_geo = (2-f_geo(i_geo))*f_geo(i_geo)/((1-f_geo(i_geo))^2);
        c_geo = a_geo(i_geo)*sqrt(1+ex2_geo);
        phi_geo = atan(Z_geo/((sqrt(X_geo^2+Y_geo^2)*(1-(2-f_geo(i_geo)))*f_geo(i_geo))));
        h_geo = 0.1; oldh_geo = 0;
        iterations_geo = 0;
        while abs(h_geo-oldh_geo) > 1.e-12
            oldh_geo = h_geo;
            N_geo = c_geo/sqrt(1+ex2_geo*cos(phi_geo)^2);
            phi_geo = atan(Z_geo/((sqrt(X_geo^2+Y_geo^2)*(1-(2-f_geo(i_geo))*f_geo(i_geo)*N_geo/(N_geo+h_geo)))));
            h_geo = sqrt(X_geo^2+Y_geo^2)/cos(phi_geo)-N_geo;
            
            iterations_geo = iterations_geo + 1;
        ...
        phi_geo = phi_geo*180/pi;
        lambda_geo = lambda_geo*180/pi;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % disp(navSolutions_latitude(currMeasNr))
        % disp(phi_geo)
        % disp(navSolutions_longitude(currMeasNr))
        % disp(lambda_geo)
        % disp(navSolutions_height(currMeasNr))
        % disp(h_geo)
        % [navSolutions_latitude(currMeasNr), ...
        %  navSolutions_longitude(currMeasNr), ...
