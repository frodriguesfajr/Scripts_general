...
% lat = −22.9068°, lon = −43.1729°, alt ≈ 10 m
% --- Geodésicas -> ECEF (WGS-84) ---
a = 6378137.0;                 % semi-eixo maior [m]
f = 1/298.257223563;           % achatamento
e2 = f*(2-f);                  % excentricidade^2

lat = deg2rad(-22.9068);       % latitude (graus -> rad)
lon = deg2rad(-43.1729);       % longitude (graus -> rad)
h   = 10;                      % altitude [m] (ajuste se quiser)
...
% UserPosition = [ 4.2868845339e+06,  -4.0218385567e+06,  -2.4672191574e+06 ];
...
numSV = 7;
R = 2.02e7;                        % ~20 200 km
theta = rand(numSV,1)*2*pi;
phi   = acos(2*rand(numSV,1)-1)-pi/2;
corrSatPosition = [R*cos(phi).*cos(theta), ...
                   R*cos(phi).*sin(theta), ...
                   R*sin(phi)];
% return
...
gamma_guess = dpe_param.gamma_guess;
dt = dpe_param.dt;
dmax = dpe_param.dmax;
