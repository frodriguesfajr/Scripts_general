...
% Usa: SatPosition, UserPosition, c, CodePeriod, B_2, CNosim, numSV

M    = numSV;
Tcoh = CodePeriod;                      % tempo de integração coerente (1 ms no seu setup)

% Vetores linha-de-visada (u_i) e matrizes de geometria
u_mat = zeros(M,3);                     % cada linha = u_i = (p_i - p)/||p_i - p||
rho   = zeros(M,1);
for i = 1:M
...
T_geo = [u_mat, ones(M,1)];             % matriz T (M x 4) para LS/WLS
Pmat  = (1/c) * u_mat;                  % d(tau)/d(p) empilhado (M x 3)

% Escolha do W (pesos da LS); com C/N0 iguais, W=I basta.
use_WLS = false;                        % mude para true se quiser WLS por C/N0
% (Se depois você simular C/N0 diferentes por satélite, reative isso.)
...
for idx = 1:length(CNosim)
    ...
    % SNR no intervalo coerente T: SNR = (C/N0) * T
    SNR_lin = 10^((CN0dB + 10*log10(Tcoh))/10);   % mesmo conceito que você usa no ZZB

    % FIM por atraso (cada sat indep.): J_tau = 2 * B2 * SNR
    Jtau = diag( 2 * B_2 * SNR_lin * ones(M,1) ); % (M x M)

    % --- CRB: abordagem convencional (propagação via LS/WLS) ---
    if use_WLS
        % pesos proporcionais ao C/N0 (aqui todos iguais; fica I)
        w = ones(M,1); w = w / max(w);
        W = diag(w);
    else
        W = eye(M);
    end
    G    = T_geo;
    A    = (G.'*W*G);                   % (4x4)
    Ainv = inv(A);                      % ok para M>=4 e boa geometria

    % C(p,dt) >= c^2 * A^{-1} * G^T W Jtau^{-1} W G * A^{-1}
    M1        = Jtau \ (W*G);           % equivalente a inv(Jtau)*W*G, mas mais estável
    C_p_dt    = c^2 * Ainv * (G.'*W*M1) * Ainv;
    fCRB_2SP(idx) = sqrt(trace(C_p_dt(1:3,1:3))); % bound de posição (m)

    % --- CRB: DPE (aproximação “pré-correlação” coerente com seu ZZB) ---
    % J_p = P^T * Jtau * P ;  C_p = inv(J_p)
    Jp       = Pmat.' * Jtau * Pmat;    % (3x3)
    C_p      = inv(Jp);
    fCRB_DPE(idx) = sqrt(trace(C_p));   % bound de posição (m)
end

% (Opcional) juntar numa tabela com seus RMSEs e ZZBs, se houver
if exist('fZZLB_2SP','var') && exist('fZZLB_DPE','var')
    T_SUM = table(CNosim(:), RMSE_LS(:), fCRB_2SP(:), fZZLB_2SP(:), ...
                            RMSE_DPE(:), fCRB_DPE(:), fZZLB_DPE(:), ...
        'VariableNames', {'CN0_dBHz','RMSE_LS_m','CRB_2SP_m','ZZB_2SP_m', ...
                          'RMSE_DPE_m','CRB_DPE_m','ZZB_DPE_m'});
else
...
for kSV=1:numSV
    if CNo(kSV)<100
        ...
        % A = sqrt( 2*10^(CNo(kSV)/10) / fs );   % fator 2 que faltava
...
function r = correlateSignal(sigen, x)
    % x: sinal recebido (1 x NsamplesData)
    numSV          = sigen.numSV;
    ...
    for kSV = 1:numSV
        for idx_nc = 1:NC
            seg = x(1, N*(idx_nc-1)+1 : N*idx_nc);   % pega o bloco de 1 ms
            Rk  = ifft( fft(seg, N) .* conj(fft_local(kSV,:)) );
            r(kSV,:) = r(kSV,:) + abs(Rk).^2;        % não-coerente
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
