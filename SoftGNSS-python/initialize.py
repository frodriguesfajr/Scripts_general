...
class Settings(object):
    ...
    def samplesPerCode(self):
        ...
        return np.int64(np.round(self.samplingFreq / (self.codeFreqBasis / self.codeLength)))
    ...
    def probeData(self, fileNameStr=None):
        ...
        try:
            with open(fileNameStr, 'rb') as fid:
                ...
                plt.plot(1000 * timeScale[1:int(samplesPerCode / 50)], data[1:int(samplesPerCode / 50)])
                plt.axis('tight')
                plt.grid()
                ...
                #plt.show()
                #plt.subplot(2, 2, 3.5)
                plt.subplot(2, 2, 3)  # Gráfico 3
                # # ou
                # #plt.subplot(2, 2, 4)  # Gráfico 4
    ...
    def postProcessing(self, fileNameStr=None):
        ...
        try:
            with open(fileNameStr, 'rb') as fid:
                ...
                file_path = 'trackingResults_python.npy'
                ...
                try:
                    if os.path.exists(file_path):  # Verifica se o arquivo existe antes de carregar
                        #trackResults.results = np.load(file_path)
                        trackResults.results = np.load(file_path, allow_pickle=True)
                    else:
                        raise FileNotFoundError  # Força a criação do arquivo se ele não existir
                except (IOError, FileNotFoundError):
                    print(f"Arquivo '{file_path}' não encontrado. Criando novo arquivo...")
                    trackResults.track(fid)
                    np.save(file_path, trackResults.results)  # Salva o resultado do rastreamento
                    print(f'Arquivo {file_path} criado e salvo com sucesso.')                
