...
class NavigationResult(Result):
    ...
    def calculatePseudoranges(self, msOfTheSignal, channelList):
        ...
        for channelNr in channelList:
            ...
            #travelTime[channelNr] = trackResults[channelNr].absoluteSample[np.int(msOfTheSignal[channelNr])] / samplesPerCode
    ...
    def postNavigate(self):
        ...
        #print(type(trackResults.status))  
        #print(trackResults.status)  

        #if settings.msToProcess < 36000 or sum(trackResults.status != '-') < 4:
        #if settings.msToProcess < 36000 or sum(s != '-' for s in trackResults.status) < 4:
        ...
        for currMeasNr in range(np.int64(np.fix(settings.msToProcess - subFrameStart.max()) / settings.navSolPeriod)):  # Usando np.int64
            ...
            # Find pseudoranges ======================================================
            channel[0].rawP[:, currMeasNr] = self.calculatePseudoranges(subFrameStart + settings.navSolPeriod * currMeasNr, activeChnList)
    ...
    def plot(self):
        ...
        mpl.rc('font', family='serif', serif='DejaVu Serif', size=10)
        ...
        if navSolutions is not None:
            ...
            h11.plot(navSolutions[0].E - refCoord.E, '-', navSolutions[0].N - refCoord.N, '-', navSolutions[0].U - refCoord.U, '-')
            h11.legend(['E', 'N', 'U'])
            h11.set(title='Coordinates variations in UTM system', xlabel='Measurement period: %i ms' % settings.navSolPeriod, ylabel='Variations (m)')
            h11.grid()
            h11.axis('tight')
            
            #ff = plt.figure(figureNumber)
            #ff.clf()
            #ff.set_label('Navigation solutions')
            #spec2 = gs.GridSpec(2, 2)
            # the axes3d module is needed for the following line
            dummy = axes3d.Axes3D
            h31 = plt.subplot(spec[2], projection='3d')
            h31.plot((navSolutions[0].E - refCoord.E).T, (navSolutions[0].N - refCoord.N).T, (navSolutions[0].U - refCoord.U).T, '+')
            ...
            h32.plot(np.deg2rad(navSolutions[0].channel[0].az.T), 90 - navSolutions[0].channel[0].el.T)
            [h32.text(x, y, s) for x, y, s in zip(np.deg2rad(navSolutions[0].channel[0].az[:, 0]), 90 - navSolutions[0].channel[0].el[:, 0], navSolutions[0].channel[0].PRN[:, 0])]
            ...
            plt.subplots_adjust(hspace=0.5, wspace=0.3)  # Ajuste os valores conforme necessário
